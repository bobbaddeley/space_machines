...
class MACHINES_API_Response {
  
  function setup() {
    global $machines_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($machines_api->query->include) {
      $this->include_values = explode(',', $machines_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($machines_api->query->exclude) {
      $this->exclude_values = explode(',', $machines_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    if (function_exists('json_encode')) {
      ...
      if (version_compare(PHP_VERSION, '5.3') < 0) {
        ...
        if ($machines_api->query->json_encode_options) {
          $json_encode_options = $machines_api->query->json_encode_options;
        }
    ...
    if ($machines_api->query->json_unescaped_unicode) {
  ...
  function respond($result, $status = 'ok') {
    ...
    if ($machines_api->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($machines_api->query->redirect) {
      $url = $this->add_status_query_var($machines_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($machines_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($machines_api->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
