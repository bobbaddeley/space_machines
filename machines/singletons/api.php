...
class MACHINES_API {
  ...
  function __construct() {
    $this->query = new MACHINES_API_Query();
    $this->response = new MACHINES_API_Response();
    add_action('template_redirect', array(&$this, 'template_redirect'));
    add_action('admin_menu', array(&$this, 'admin_menu'));
    add_action('update_option_machines_api_base', array(&$this, 'flush_rewrite_rules'));
    add_action('pre_update_option_machines_api_controllers', array(&$this, 'update_controllers'));
  }
  ...
  function template_redirect() {
    ...
    $enabled_controllers = explode(',', get_option('machines_api_controllers', 'core'));
    $enabled_ip_addresses = preg_split("/[\s,]+/", get_option('machines_ip_addresses', ''));
    ...
    if ($controller) {
      ...
      //check the enabled IP addresses to make sure the referrer is allowed.
	  //if not, bail.
	  if (sizeof($enabled_ip_addresses)>0 && $enabled_ip_addresses[0]!=""){
		$ip = $_SERVER['REMOTE_ADDR'];
		if ($this->check_ip_address($enabled_ip_addresses, $ip)==false){
			$this->error("IP Address Not Approved");
		}
	  }
      ...
      if ($method) {
        ...
        do_action("machines_api", $controller, $method);
        do_action("machines_api-{$controller}-$method");
  ...
  function check_ip_address($list, $input_ip){
	$in_ip = ip2long($input_ip);
	foreach($list as $ip){
  ...
  function admin_menu() {
    add_options_page('Machines API Settings', 'Machines API', 'manage_options', 'machines-api', array(&$this, 'admin_options'));
  }
  ...
  function admin_options() {
    ...
    if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
      if ((!empty($_REQUEST['action']) || !empty($_REQUEST['action2'])) &&
        ...
        $this->save_option('machines_api_controllers', implode(',', $active_controllers));
      }
      if (isset($_REQUEST['machines_api_base'])) {
        $this->save_option('machines_api_base', $_REQUEST['machines_api_base']);
      }
      if (isset($_REQUEST['machines_ip_addresses'])) {
        $this->save_option('machines_ip_addresses', $_REQUEST['machines_ip_addresses']);
      }
    }
...
<div class="wrap">
  ...
  <h2>Machines API Settings</h2>
  <form action="options-general.php?page=machines-api" method="post">
          ...
          <tr class="<?php echo ($active ? 'active' : 'inactive'); ?>">
            ...
            <td class="plugin-title">
              ...
              <div class="row-actions-visible">
                ...
                if ($active) {
                  echo '<a href="' . wp_nonce_url('options-general.php?page=machines-api&amp;action=deactivate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Deactivate this controller') . '" class="edit">' . __('Deactivate') . '</a>';
                } else if (!$error) {
                  echo '<a href="' . wp_nonce_url('options-general.php?page=machines-api&amp;action=activate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Activate this controller') . '" class="edit">' . __('Activate') . '</a>';
                }
    ...
    <h3>Address</h3>
    <p>Specify a base URL for Machines API. For example, using <code>api</code> as your API base URL would enable the following <code><?php bloginfo('url'); ?>/api/get_recent_posts/</code>. If you assign a blank value the API will only be available by setting a <code>json</code> query variable.</p>
    <table class="form-table">
      <tr valign="top">
        <th scope="row">API base</th>
        <td><code><?php bloginfo('url'); ?>/</code><input type="text" name="machines_api_base" value="<?php echo get_option('machines_api_base', 'api'); ?>" size="15" /></td>
      </tr>
    </table>
    ...
    <?php } ?>
	<h3>Permitted IPs</h3>
	<p>If blank, all incoming IPs are acceptable. Use this field to limit API requests by IP address. This is useful to prevent API calls from outside the location you're trying to manage.
	Use one line per address, asterisk for wildcard</p>
	<textarea name="machines_ip_addresses"><?php echo get_option('machines_ip_addresses', ''); ?></textarea>
    <p class="submit">
...
<?php
  ...
  function get_controllers() {
    ...
    $controllers = apply_filters('machines_api_controllers', $controllers);
  ...
  function check_directory_for_controllers($dir, &$controllers) {
    ...
    while ($file = readdir($dh)) {
      if (preg_match('/(.+)\.php$/i', $file, $matches)) {
        ...
        if (preg_match("/class\s+MACHINES_API_{$matches[1]}_Controller/i", $src)) {
  ...
  function controller_is_active($controller) {
    if (defined('MACHINES_API_CONTROLLERS')) {
      $default = MACHINES_API_CONTROLLERS;
    } else {
      $default = 'core';
    }
    $active_controllers = explode(',', get_option('machines_api_controllers', $default));
  ...
  function controller_class($controller) {
    return "machines_api_{$controller}_controller";
  }
  ...
  function controller_path($controller) {
    ...
    if (file_exists($theme_path)) {
      $path = $theme_path;
    } else if (file_exists($machines_api_path)) {
  ...
  function get_nonce_id($controller, $method) {
    ...
    return "machines_api-$controller-$method";
  }
